...
Human-readable mirror of the per-command JSON shapes the CLI emits on stdout. The live source of truth is `OUTPUT_SHAPES` in `src/commands/schema.ts`. The CLI exposes the same data at runtime via `posthog schema` and `posthog <cmd> --help --json`.
...
When you change a command's output, update both `OUTPUT_SHAPES` in `src/commands/schema.ts` AND this doc.
